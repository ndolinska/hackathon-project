...
const config = {
    ...
    width: 600,
    height: 800,
    parent: 'game-container', // Phaser wrzuci canvas do naszego retro-diva
    backgroundColor: '#f8d6a3', // Klasyczny zielony z Gameboya
    pixelArt: true, // KLUCZOWE: Zapobiega rozmywaniu pikseli!
    scene: {
        preload: preload,
        create: create,
        update: update
...
let stats = {
    hunger: 50,
    happiness: 40,
    trust: 15 // Lejek adopcyjny
};

function preload() {
    // Miejsce na load prawdziwych pixel-artów (np. sprite'sheetów)
}

function create() {
    // Style tekstów używające retro fontu z CSS
    ...
    // 2. Pikselowy pseudo-kot (Złożony z figur geometrycznych jako placeholder)
   const catContainer = this.add.container(300, 450);
    catContainer.add(this.add.rectangle(0, 0, 90, 80, colors.darkest)); // Nieco większy kot
    ...
    // Animacja "klatkowa" (skokowa), imitująca małą liczbę klatek na sekundę z lat 90.
   this.tweens.add({
...
function update() {
    // Logika spadających statystyk w czasie
}
...
function createButton(scene, x, y, text, color, fontSize = 12) {
    // Cień przycisku (dla efektu 3D/wciśnięcia)
